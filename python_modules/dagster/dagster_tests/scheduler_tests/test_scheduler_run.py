...
def test_status_in_code_schedule(instance: DagsterInstance, executor: ThreadPoolExecutor):
    ...
    with create_test_daemon_workspace_context(
        ...
        with pendulum.test(freeze_datetime):
            ...
            assert instigator_state.status == InstigatorStatus.DECLARATIVE
            assert (
...
def test_change_default_status(instance: DagsterInstance, executor: ThreadPoolExecutor):
    ...
    with create_test_daemon_workspace_context(
        ...
        with pendulum.test(freeze_datetime):
            ...
            # DECLARATIVE row has been removed from the database
            instigator_state = instance.get_instigator_state(
                never_running_origin.get_id(), not_running_schedule.selector_id
            )
...
class TestSchedulerRun:
    ...
    def test_multi_runs(
        ...
        with pendulum.test(freeze_datetime):
            ...
            runs = scheduler_instance.get_runs()
            validate_run_started(
                scheduler_instance, runs[0], execution_time=create_pendulum_time(2019, 2, 28)
            )
            validate_run_started(
                scheduler_instance, runs[1], execution_time=create_pendulum_time(2019, 2, 28)
            )
    ...
    def test_asset_selection(
        ...
        with pendulum.test(freeze_datetime):
            ...
            validate_run_started(
                scheduler_instance, run, execution_time=create_pendulum_time(2019, 2, 28)
            )
    ...
    def test_stale_asset_selection_never_materialized(
        ...
        with pendulum.test(freeze_datetime):
            ...
            validate_run_started(
                scheduler_instance, schedule_run, execution_time=create_pendulum_time(2019, 2, 28)
            )
